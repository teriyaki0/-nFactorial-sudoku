 ...
 */
function cellAvailable(tempInitArray, box, value) {
  return tempInitArray[getIndexOfCell(box, value)] === "0" ? 0 : 1;
}

/**
 * Generates a Unique Sudoku puzzle from a solved Sudoku.
 */
function generateUniqueSudoku(solvedArray, difficulty, e) {
  ...
  for (let j = 0; j < 9; j++) {
    boxCounts[j] =
      cellAvailable(tempInitArray, j, 0) +
      cellAvailable(tempInitArray, j, 1) +
      cellAvailable(tempInitArray, j, 2) +
      cellAvailable(tempInitArray, j, 3) +
      cellAvailable(tempInitArray, j, 4) +
      cellAvailable(tempInitArray, j, 5) +
      cellAvailable(tempInitArray, j, 6) +
      cellAvailable(tempInitArray, j, 7) +
      cellAvailable(tempInitArray, j, 8);
  }
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    cellsAvailable = [];
    for (let j = 0; j < 9; j++) {
      if (tempInitArray[getIndexOfCell(box, j)] === "0") {
...
export const getUniqueSudoku = (difficulty, e) => {
   ...
   */
  temporaryInitArray = generateUniqueSudoku(
